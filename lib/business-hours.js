...
function getTimes(hours, d){ // only used for ship times
    if(!hours[d.day()]) return null
    return hours[d.day()].map(function (time) {
        time = time.split(':');
        var _d = d.clone();
        _d.startOf("day")
        _d.add(time[0], "hours");
        _d.add(time[1] || 0, "minutes");
        _d.add(time[2] || 0, "seconds");
        _d.milliseconds(0);
        return _d;
    });
  }
function shippingTimes(d) {
    d = d.clone();
    var hours = getLocaleData('shippinghours');
    if (!d.isShipDay()) {
        return null;
    }

    var times = getTimes(hours, d);

    if(d.isBefore(times[0])){
      var c = moment(d).subtract(1, "day")
      var dayBefore = getTimes(hours, c)
      if(dayBefore && dayBefore[1].isAfter(d)){
        return dayBefore
      }
    }
    return times
...
moment.fn.isShipDay = function isShipDay() {
    var hours = getLocaleData('shippinghours');
    return !!hours[this.day()] && !this.isHoliday();
};
...
moment.fn.nextShipDay = function nextShipDay() {
    var d = this.clone();
    d = d.add(1, 'day');
    while (!d.isShipDay()) {
        d = d.add(1, 'day');
...
moment.fn.lastShipDay = function lastShipDay() {
    var d = this.clone();
    d = d.subtract(1, 'day');
    while (!d.isShipDay()) {
        d = d.subtract(1, 'day');
...
moment.fn.nextShippingTime = function nextShippingTime() {
    if (this.isShipDay()) {
        var shippinghours = shippingTimes(this);
        if (this.isBefore(shippinghours[0])) {
            return shippinghours[0];
        } else if (this.isAfter(shippinghours[1])) {
            return shippingTimes(this.nextShipDay())[0];
        } else {
            return this.clone();
        }
    } else {
...
moment.fn.lastShippingTime = function lastShippingTime() {
    if (this.isShipDay()) {
        var shippinghours = shippingTimes(this);
        if (this.isAfter(shippinghours[1])) {
            return shippinghours[1];
        } else if (this.isBefore(shippinghours[0])) {
            return shippingTimes(this.lastShipDay())[1];
        } else {
            return this.clone();
        }
    } else {
    ...
    if (unit === 'day') {
        ...
        if (detail) {
            ...
            var hours = from.diff(to, 'hour', true),
                denominator = from.isWorkingDay() ? from : from.nextWorkingDay(),
